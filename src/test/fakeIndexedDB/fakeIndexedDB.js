...
describe("fakeIndexedDB Tests", () => {
    ...
    it("properly handles compound keys (issue #18)", (done) => {
        ...
        request.onsuccess = (event) => {
            ...
            tx.objectStore("books").openCursor(["Fred", 123456]).onsuccess = (event2) => {
        ...
        function read() {
            ...
            return new Promise((resolve) => {
                fakeIndexedDB.open("mydb").onsuccess = (event) => {
                    ...
                    const store = tx.objectStore("mystore");
                    store.getAll().onsuccess = (event2) => resolve(event2.target.result);
                };
            });
        }
        await setup();
    ...
    it("FDBObjectStore.delete works with a key range (issue #53)", (done) => {
        ...
        openreq.onsuccess = (event) => {
            const db = event.target.result;
            db.transaction("items").objectStore("items").count().onsuccess = (event2) => {
