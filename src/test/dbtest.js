...
async function initializeDB(fakeIndexedDB) {
...
async function deleteDatabase(fakeIndexedDB) {
    return new Promise((resolve, reject) => {
...
async function testRead(db) {
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = (event) => {
            ...
            logTestResult("Read Test - Quarry Memories", passed, passed
                ? undefined
                : `Expected "Quarry Memories", got "${book?.title || "undefined"}"`);
            resolve();
        };
        tx.onerror = (event) => reject(new Error(`Transaction failed: ${event.target.error}`));
    });
}
    ...
    return new Promise((resolve, reject) => {
        ...
        addRequest.onsuccess = () => {
            ...
            getRequest.onsuccess = (event) => {
                ...
                logTestResult("Create Test - Modern Stone Tools", passed, passed ? undefined : "Book was not found after creation");
                resolve(passed);
            };
        };
        tx.onerror = (event) => reject(new Error(`Transaction failed: ${event.target.error}`));
    ...
    return new Promise((resolve, reject) => {
        ...
        updateRequest.onsuccess = () => {
            ...
            getRequest.onsuccess = (event) => {
                ...
                logTestResult("Update Test - Water Buffaloes", passed, passed
                    ? undefined
    ...
    return new Promise((resolve, reject) => {
        ...
        deleteRequest.onsuccess = () => {
            ...
            getRequest.onsuccess = (event) => {
                ...
                logTestResult("Delete Test - Stone Age Cooking", passed, passed ? undefined : "Book still exists after deletion");
                resolve();
            };
        };
        tx.onerror = (event) => reject(new Error(`Transaction failed: ${event.target.error}`));
...
async function testIndexQueries(db) {
    return new Promise((resolve, reject) => {
        const tx = db.transaction("books", "readonly");
        const store = tx.objectStore("books");
        ...
        let testsPassed = 0;
        // Test author index
        store.index("by_author").getAll("Fred").onsuccess = (event) => {
            const books = event.target.result;
            const passed = books.length === 2;
            testsPassed += passed ? 1 : 0;
            testsCompleted++;
            logTestResult('Index Query - Author "Fred"', passed, passed ? undefined : `Expected 2 books, got ${books.length}`);
        };
        // Test title index
        store.index("by_title").get("Bedrock Nights").onsuccess = (event) => {
            const book = event.target.result;
            const passed = book && book.author === "Barney";
            testsPassed += passed ? 1 : 0;
            testsCompleted++;
            logTestResult('Index Query - Title "Bedrock Nights"', passed, passed
                ? undefined
                : `Expected author "Barney", got "${book?.author || "undefined"}"`);
        };
        tx.oncomplete = () => {
            logTestResult("All Index Queries", testsPassed === 2, `${testsPassed}/${testsCompleted} passed`);
            resolve();
        };
        tx.onerror = (event) => reject(new Error(`Transaction failed: ${event.target.error}`));
    });
}
async function testRangeQueries(db, IDBKeyRange) {
    return new Promise((resolve, reject) => {
        const tx = db.transaction("books", "readonly");
        const store = tx.objectStore("books");
        let booksInRange = 0;
        const priceRange = IDBKeyRange.bound(20, 35);
        store.index("by_price").openCursor(priceRange).onsuccess = (event) => {
            const cursor = event.target.result;
            if (cursor) {
                booksInRange++;
                console.log(`  📚 Found in range: "${cursor.value.title}" ($${cursor.value.price})`);
                cursor.continue();
            }
            else {
                const passed = booksInRange > 0;
                logTestResult("Range Query - Price $20-$35", passed, `Found ${booksInRange} books in range`);
                resolve();
            }
        };
        tx.onerror = (event) => reject(new Error(`Transaction failed: ${event.target.error}`));
...
async function testPersistenceEdgeCases(fakeIndexedDB) {
...
async function runTests() {
    ...
    try {
        ...
        const currentCycle = await new Promise((resolve) => {
            if (!db)
        ...
        switch (nextCycle) {
            case 1:
                ...
                break;
            case 2:
                tests.push({ name: "Verify Persistence", fn: testRead }, { name: "Modify Data", fn: testUpdate });
                break;
            case 3:
                tests.push({ name: "Verify Updates", fn: testRead }, { name: "Query Tests", fn: testIndexQueries }, { name: "Range Tests", fn: testRangeQueries });
                break;
            case 4:
        ...
        await new Promise((resolve, reject) => {
            ...
            request.onerror = (event) => {
                reject(new Error(`Failed to save cycle: ${event.target.error}`));
            };
