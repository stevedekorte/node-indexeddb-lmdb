...
class ObjectStore {
    ...
    constructor(rawDatabase, name, keyPath, autoIncrement) {
        ...
        const keyPrefix = PathUtils.createObjectStoreKeyPath(this.rawDatabase.name, this.name);
        ...
        if (dbStructure && dbStructure.objectStores[name]) {
            ...
            for (const [indexName, indexData] of Object.entries(osData.indexes)) {
                const index = new Index(this, indexName, indexData.keyPath, indexData.multiEntry, indexData.unique);
                index.initialized = true;
                this.rawIndexes.set(indexName, index);
            }
        }
        if (process.env.DB_VERBOSE === "1") {
            console.log(this.rawDatabase.name, this.name, "rawIndexes", this.rawIndexes);
        }
    }
    ...
    async saveStructure() {
        ...
        let dbStructure = dbManager.getDatabaseStructure(this.rawDatabase.name);
        if (!dbStructure) {
    ...
    async deleteRecord(key, rollbackLog) {
